...
def determinar_tipo(dados):
    """Função para determinar se é Filial ou Matriz."""
    tipo = dados.get('tipo', '')
    return tipo.capitalize() if tipo else 'Não Especificado'
        ...
        with open(arquivo, 'a', newline='', encoding='utf-8') as file:
            ...
            if file.tell() == 0:  # Verifica se o arquivo está vazio para escrever o cabeçalho
                writer.writerow([
                    ...
                    'Natureza Jurídica', 'Natureza Tipo', 'Tipo', 'Atividade Principal', 
                    'Município', 'UF', 'Inscrição Estadual', 'Regime', 'Ativo', 'Estado'
                ])
            
            est = dados.get('estabelecimento', {})
            cidade = est.get('cidade', {})
            estado = est.get('estado', {})
            inscricao_estadual = est.get('inscricao_estadual', '')
            regime = est.get('regime', '')
            ativo = est.get('ativo', '')
            ...
            writer.writerow([
                ...
                regime,
                ativo,
                estado.get('descricao', '')  # Adiciona a descrição do estado
            ])
    except Exception as e:
        print(f"Erro ao salvar dados no CSV: {e}")

def menu():
    arquivo_csv = 'cnpj8.csv'
    while True:
        print("\nMenu")
