    ...
    try:
        with open(arquivo, 'a', newline='', encoding='utf-8') as file:
            ...
            writer.writerow([
                ...
                inscricoes_estaduais.get('inscricao_estadual', '') if inscricoes_estaduais else '',  # Verifica se inscricoes_estaduais não é None
                simples.get('regime', '') if simples else '',  # Verifica se simples não é None
                'Sim' if inscricoes_estaduais.get('ativo', False) else 'Não',  # Verifica se a inscrição está ativa
                estado.get('nome', '') if estado else ''  # Adiciona a descrição do estado se não for None
            ])
